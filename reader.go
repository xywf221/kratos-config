...
func (r *reader) Merge(kvs ...*KeyValue) error {
	...
	for _, kv := range kvs {
		...
		if err := r.opts.decoder(kv, next); err != nil {
			return fmt.Errorf("failed to config decode error: %v key: %s value: %s", err, kv.Key, string(kv.Value))
		}
		if err := mergo.Map(&merged, convertMap(next), mergo.WithOverride); err != nil {
			return fmt.Errorf("failed to config merge error: %w key: %s value: %s", err, kv.Key, string(kv.Value))
		}
	}
	r.lock.Lock()
	r.values = merged
