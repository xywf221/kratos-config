...
import (
	...
	// init encoding
	_ "github.com/xywf221/kratos-config/encoding/json"
	_ "github.com/xywf221/kratos-config/encoding/xml"
	_ "github.com/xywf221/kratos-config/encoding/yaml"
)
...
func (c *config) Load() error {
	for _, src := range c.opts.sources {
		...
		if err = c.reader.Merge(kvs...); err != nil {
			return fmt.Errorf("failed to merge config source: %w", err)
		}
		w, err := src.Watch()
		if err != nil {
	...
	if err := c.reader.Resolve(); err != nil {
		return fmt.Errorf("failed to resolve config source: %w", err)
	}
	return nil
}
